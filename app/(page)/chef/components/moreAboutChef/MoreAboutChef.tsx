...
export default function MoreAboutUs({ role, chefName, imageUrl, chefLink, faceBookLink, twitterLink, youtubeLink, linkedinLink }: PropsType) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1 }} onMouseEnter={() => setonAction(true)} onMouseLeave={() => setonAction(false)} className=" w-92 flex flex-wrap  h-[31.25rem]  ">
      <div className="h-[400px] w-full   relative">
        <div
          className={`${onAction ? "rotate-3" : "rotate-0"} rounded-xl absolute top-0 left-0 w-70 sm:w-86 h-96 
       border-2 border-dotted border-green-500 transition-all ease-in-out duration-500`}
        ...
        <div
         ...
         right-0 rounded-xl  w-65 sm:w-86 h-96 `}
        ></div>
