...
export default function FoodBox({ bg, box, color = "text-[#fff]", price = "", colorPrice = "text-[#212121]" }: propsType) {
  return (
    <div dir="rtl" className=" w-full md:w-124  relative  p-12 text-white h-100 bg_home ">
      <div>
        ...
        <ReusableOrderButton icon={<FaTruckFast size={22} className="transition-all duration-500" />} className={`mt-6 ${bg} ${color} relative overflow-hidden outline-none  cursor-pointer  bg-[#D12525] text-white font-bold w-60  md:w-68 py-4 pb-5 px-4 flex  items-center justify-center gap-2 group h-16`} hoverBgColor="group-hover:bg-[#ffff]" textColor="text-white" hoverTextColor="group-hover:text-black" rounded={true} fullWidth={false} text="اکنون سفارش دهید" />
      </div>
