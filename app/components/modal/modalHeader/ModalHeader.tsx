...
export default function ModalHeader({ setOpen }: PropsType) {
  ...
  return (
    <div className="p-4 pb-[2rem] w-full h-full bg-white rounded-lg pt-[2rem] ">
      <div className=" flex justify-between items-center ">
        <button onClick={() => setOpen(false)} className=" bg-red-500 text-white px-[1rem] py-[.4rem] cursor-pointer rounded-full text-2xl font-bold mb-4">
          ×
        </button>
        <img className=" " src="https://t-theme.com/foodking/wp-content/uploads/2024/08/U_U_O¯U©U_U_U¯_14-transformed.png" alt="" />
      </div>
      <div dir="rtl" className=" text-xl font-normal mt-[2rem] mb-[6rem]  flex-col-reverse flex text-[#5C5C5B] gap-[2rem]  ">
        ...
        >
          ...
          <AnimatePresence>
            {showBox3 && (
              <motion.div className="absolute right-[0rem] shadow-md top-[2.5rem] border-[1px] border-[#eeeeee] bg-[#ffffff] rounded-md w-[14rem] overflow-hidden" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
                <ul className="flex flex-col text-right text-[#212121]   ">
        ...
        <Link className="   cursor-pointer   gap-[.2rem] " href={"/"}>
          <span className=" hover:text-[#00813D] transition ease-in delay-150 font-medium"> صفحه اصلی</span>
        </Link>
      </div>
      <div dir="rtl" className="  font-semibold mt-[2rem] text-lg flex flex-col gap-[1rem] text-[#212121] ">
        <h2 className=" mb-[1.5rem] font-semibold text-2xl ">اطلاعات تماس</h2>
        ...
        <button className=" w-full h-16 md:h-16 mb-[1.5rem] group relative font-semibold rounded-xl text-white bg-[#00813D] ">
        ...
        <div className="   mb-[1rem] flex gap-[1.5rem]">
          <div className="group bg-[#5c5c5b1a] h-[2.5rem] w-[2.5rem] rounded-full cursor-pointer flex items-center justify-center transition-colors duration-200 hover:bg-[#D12525]">
