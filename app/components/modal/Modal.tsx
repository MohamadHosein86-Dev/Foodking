...
const Modal = ({ isOpen, onClose, children, center }: ModalProps) => {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[1000000]" initial={{ opacity: 0.5 }} animate={{ opacity: 1 }} exit={{ opacity: 0.25 }} onClick={onClose}>
